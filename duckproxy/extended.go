...
func (s *session) handleParse(ctx context.Context, msg *pgproto3.Parse) {
	if existing, ok := s.statements[msg.Name]; ok {
		existing.close()
		delete(s.statements, msg.Name)
	}

	stmt, err := prepareStatement(ctx, s.dconn, msg.Name, msg.Query, msg.ParameterOIDs)
	if err != nil {
		if s.tx != txIdle {
			s.tx = txFailed
		}
...
func (s *session) handleBind(msg *pgproto3.Bind) {
	...
	args := make([]driver.NamedValue, len(msg.Parameters))
	for i, raw := range msg.Parameters {
		...
		args[i] = driver.NamedValue{Ordinal: i + 1, Value: val}
	}

	if err := stmt.stmt.Bind(args); err != nil {
		if s.tx != txIdle {
			s.tx = txFailed
		}
		s.backend.Send(toErrorResponse(err))
...
func (s *session) handleClose(msg *pgproto3.Close) {
	switch msg.ObjectType {
	case 'S':
		if stmt, ok := s.statements[msg.Name]; ok {
			stmt.close()
			delete(s.statements, msg.Name)
		}
	case 'P':
