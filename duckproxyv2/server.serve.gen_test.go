...
func TestServerServe(t *testing.T) {
	...
	t.Run("concurrent_clients_bounded_by_max_connections", func(t *testing.T) {
		...
		socketPath, serverDB := startTestServer(t, duckproxyv2.WithMaxConnections(maxConnections))
		...
		for i := range clients {
			...
			go func(client int) {
				...
				// a free slot.
				clientDB, err := sql.Open("duckproxyv2", socketPath)
				if err != nil {
					mu.Lock()
		...
		for _, err := range errs {
			if strings.Contains(err.Error(), "already in use") || strings.Contains(err.Error(), "lock") {
				t.Errorf("lock contention error (the bug duckproxyv2 fixes): %v", err)
			} else {
