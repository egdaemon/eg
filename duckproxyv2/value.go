...
func toProtoValue(v driver.Value) (*Value, error) {
	...
	default:
		return nil, fmt.Errorf("duckproxyv2: unsupported value type %T", v)
	}
}
...
func fromProtoValue(v *Value) (driver.Value, error) {
	...
	case *Value_BignumValue:
		...
		if !ok {
			return nil, fmt.Errorf("duckproxyv2: invalid bignum value %q", k.BignumValue)
		}
		return bi, nil
	...
	case *Value_StructValue:
		...
		return out, nil
	case nil:
		return nil, fmt.Errorf("duckproxyv2: empty Value")
	default:
		return nil, fmt.Errorf("duckproxyv2: unknown Value kind %T", k)
	}
}
