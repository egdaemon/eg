...
func TestShellCommands(t *testing.T) {
	...
	examples := []example{
		...
		},
		{
			...
			expected:    "/workspace:CC=gcc:CFLAGS=-O2:sudo:-E -H -u egd -g egd bash -c make build",
		},
		{
			...
			expected:    ":SYSTEMD_LOG_LEVEL=debug:sudo:-E -H -u root -g root bash -c systemctl restart nginx",
		},
		{
			...
			expected:    "/tmp:PAGER=:PGPASSWORD=secret:sudo:-E -H -u postgres -g postgres bash -c psql -c \"SELECT 1\"",
		},
	}
	...
	t.Run("environ carries through to derived commands", func(t *testing.T) {
		...
		require.Equal(t, ":PAGER=:PGPASSWORD=secret:sudo:-E -H -u postgres -g postgres bash -c psql --version", rec.Result())
	})
	...
	t.Run("derived commands get independent environ copies", func(t *testing.T) {
		...
		require.Equal(t, ":APP_ENV=production:EXTRA=one:sudo:-E -H -u deploy -g deploy bash -c echo first", rec1.Result())

		rec2 := shell.NewRecorder(&cmd2)
		require.NoError(t, shell.Run(ctx, cmd2))
		require.Equal(t, ":APP_ENV=production:EXTRA=two:sudo:-E -H -u deploy -g deploy bash -c echo second", rec2.Result())
	})
	...
	t.Run("derived command does not mutate template environ", func(t *testing.T) {
		...
		rec := shell.NewRecorder(&cmd)
		require.NoError(t, shell.Run(ctx, cmd))
		require.Equal(t, ":BASE=value:sudo:-E -H -u egd -g egd bash -c echo original", rec.Result())
	})
	...
	t.Run("multiple commands record sequentially", func(t *testing.T) {
		...
		require.Equal(t, ":PAGER=:sudo:-E -H -u postgres -g postgres bash -c pg_isready", rec1.Result())
		require.Equal(t, ":PAGER=:sudo:-E -H -u postgres -g postgres bash -c psql -c \"SELECT 1\"", rec2.Result())
	})
}
