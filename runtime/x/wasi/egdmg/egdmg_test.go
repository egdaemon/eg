...
func TestBuild(t *testing.T) {
	t.Run("example1", func(t *testing.T) {
		testx.PrivateTemp(t)
		r := &shell.Recorder{}
		rt := shell.Runtime().UnsafeExec(r.Record)
		...
		require.NoError(t, fsx.SymlinkExists(egenv.EphemeralDirectory("Applications")))
		...
		require.True(t, strings.HasPrefix(r.Result(), ":DMG_VOLUME_NAME=eg.amd64:DMG_OUTPUT=eg.amd64.dmg:sudo:-E -H -u egd -g egd bash -c mkisofs -V ${DMG_VOLUME_NAME} -D -R -apple -no-pad -o ${DMG_OUTPUT} "))
	})
}
