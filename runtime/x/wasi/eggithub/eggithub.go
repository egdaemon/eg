...
// Release to github, this is very experimental. it will delete any
...
// Release2 to github, this is very experimental.
// WARNING: for local environments this assumes you've provided the token to the eg command.
// e.g.) GH_TOKEN="$(gh auth token)" eg compute local -e GH_TOKEN
// WARNING: for hosted environments: we've assumed the git auth access token for pulling the repository
// will work. this has not yet been validated. and likely needs permission updates.
func Release2(patterns ...string) eg.OpFn {
	return func(ctx context.Context, o eg.Op) error {
		c := eggit.EnvCommit()
		version := PatternVersion()
		...
		return eg.Sequential(
			eg.WhenFn(func(ctx context.Context) bool {
				return shell.Run(ctx, runtime.Newf("gh release view %s", version)) != nil
			}, shell.Op(
				runtime.Newf("gh release create --target %s %s", c.Hash.String(), version),
			)),
